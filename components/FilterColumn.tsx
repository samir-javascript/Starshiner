...
const FilterColumn = () => {
  ...
  // Function to generate search parameters
  const generateSearchParams = (colors: string[], categories: string[], sort: string) => {
    const params = new URLSearchParams();
    colors.forEach((color) => {
      params.append("color", color);
    });
  ...
  useEffect(() => {
    const paramsString = generateSearchParams(checkedColors, checkedCategories, sortOrder);
    setSearchParams(paramsString);
